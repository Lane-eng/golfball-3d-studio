...
ballMaterial.normalScale.set(0.035, 0.035);
ballMaterial.roughnessMap = ballRoughnessTexture;
ballMaterial.envMapIntensity = 1.55;
...
async function loadCoverSet(name) {
  if (name === 'clean') {
    ...
    ballMaterial.normalScale.set(.035, .035);
...
// Empty placeholder only. The app never renders a generated golf-ball sphere.
const ball = new THREE.Mesh(new THREE.BufferGeometry(), ballMaterial);
ball.visible = false;
...
async function applyBallModel(model) {
  ...
  const label = path.includes('uploaded-ball-detailed') ? 'Imported detailed hero mesh'
    : path.includes('uploaded-ball-performance') ? 'Imported performance mesh'
    : path.includes('high') ? 'Generated high-detail mesh'
    : path.includes('mobile') ? 'Generated mobile mesh'
    : 'Generated balanced mesh';
  ...
  try {
    ...
    if (!sourceMesh) {
      throw new Error(`No mesh geometry was found inside ${path}`);
    }

    const geometry = sourceMesh.geometry.clone();

    // Preserve the model's authored UVs and normals. Only compute normals if absent.
    ...
    document.getElementById('activeMeshReadout').textContent =
      `${label} · ${Math.round(triangles).toLocaleString()} triangles`;

    setStatus(`${label} loaded successfully.`);
  } catch (error) {
    console.error(error);
    ball.visible = false;
    document.getElementById('activeMeshReadout').textContent = 'MODEL LOAD FAILED';
    setStatus(`Model failed to load: ${error.message}`);
    showPreviewError(
      `The selected imported golf-ball mesh did not load.\n\n` +
      `${path}\n\n` +
      `This build intentionally has no cartoon/procedural fallback, so a missing asset cannot be mistaken for the real model.`
    );
  }
}
...
function applyLightingPreset(name) {
  if (name === 'productDark') {
    ...
    keyLight.position.set(-4.5, 5.5, 4.2);
    fillLight.intensity = 1.35;
    fillLight.position.set(4.8, 1.8, 3.2);
    rimLight.intensity = 3.1;
    rimLight.position.set(2.5, 4.2, -4.8);
    ballMaterial.envMapIntensity = 1.65;
    renderer.toneMappingExposure = 1.03;
    setStatus('Realistic dark product lighting ready.');
    return;
  }
...
applyBallModel('uploadedDetailed');
applyFinish('satin');
applyLightingPreset('productDark');
