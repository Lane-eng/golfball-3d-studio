...
function createDimpledBallGeometry(count = 360, dimpleRadius = 0.083, depth = 0.018) {
  // Real vertex displacement rather than a stretched UV normal-map texture.
  // The relatively dense sphere keeps the dimple rims round while preserving UVs
  // for front, rear, and stripe personalization artwork.
  const geometry = new THREE.SphereGeometry(1, 256, 160);
  const position = geometry.attributes.position;
  const centers = fibonacciSpherePoints(count);
  const cosRadius = Math.cos(dimpleRadius);
  ...
  for (let i = 0; i < position.count; i++) {
    ...
    for (let j = 0; j < centers.length; j++) {
      const dot = vertex.dot(centers[j]);
      if (dot <= cosRadius) continue;

      const angle = Math.acos(THREE.MathUtils.clamp(dot, -1, 1));
      const normalized = angle / dimpleRadius;
      // Smooth concave bowl: deepest at center and tangent at the rim.
      const bowl = Math.pow(1 - normalized * normalized, 2);
      if (bowl > deepest) deepest = bowl;
    }

    const radius = 1 - depth * deepest;
    position.setXYZ(i, vertex.x * radius, vertex.y * radius, vertex.z * radius);
  }
...
function applyBallModel(model) {
  ...
  if (model === 'blankLowDimple') {
    count = 290;
    radius = 0.091;
    depth = 0.020;
  }
  if (model === 'blankHighDimple') {
    count = 430;
    radius = 0.074;
    depth = 0.016;
  }

  setStatus(`Building ${count}-dimple ball…`);

  // Let the status paint before doing the one-time geometry calculation.
  requestAnimationFrame(() => {
    const nextGeometry = createDimpledBallGeometry(count, radius, depth);
    const previousGeometry = ball.geometry;
    ball.geometry = nextGeometry;
    ballGeometry = nextGeometry;
    previousGeometry?.dispose();
    ballMaterial.normalMap = null;
    ballMaterial.bumpMap = null;
    ballMaterial.needsUpdate = true;
    setStatus(`${count}-dimple generic ball ready.`);
  });
}
